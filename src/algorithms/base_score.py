...
def score_worm_considerations_isolated(
        colour: Optional[float], mask: WormMask, outer_mask: WormMask) -> Optional[tuple[float, float, float, float]]:
    """
    ...
    return 0.5 * area_score, body_score, 0.25 * edge_score, 2 * consistency_score
...
def score_worm_isolated(colour: Optional[float], mask: WormMask, outer_mask: WormMask) -> float:
    ...
    Does not consider other worms in any clew that the worm is a part of.
    """
    considerations = score_worm_considerations_isolated(colour, mask, outer_mask)
